        ...
        'uni-fab--rightTop': rightTop
		  ...
		  'uni-fab__content--other-platform': !isAndroidNvue
				...
				<view v-for="(item, index) in content" :key="index" :class="{ 'uni-fab__item--active': isShow }" class="uni-fab__item" @click="_onItemClick(index, item)">
					<image v-if="index!=2" :src="item.active ? item.selectedIconPath : item.iconPath" class="uni-fab__item-image" mode="widthFix" />
					<text  v-if="index!=2" class="uni-fab__item-text" :style="{ color: item.active ? styles.selectedColor : styles.color }">{{ item.text }}</text>
					<image v-if="index==2" :src="item.active ? item.iconPath : item.iconPath" class="uni-fab__item-image" mode="widthFix" />
					<text  v-if="index==2" class="uni-fab__item-text" :style="{ color: item.active ? styles.color: styles.color }">{{ item.text }}</text>
